...
export default function InvitacionDetail() {
	...
	const { data, error, isLoading } = useSWR<Invitacion>(`/invitaciones/${token}/qr`, fetcher)
	...
	if (error) {
		return (
			<main className="min-h-screen flex items-center justify-center p-6 md:p-8">
				<Background />
				<p>Error al cargar la invitación. Por favor, inténtalo de nuevo más tarde.</p>
			</main>
		)
	}
		...
		<div className="min-h-screen flex flex-col items-center justify-center p-6 md:p-8">
			...
			<div className="max-w-2xl w-full bg-white rounded-xl shadow-xl overflow-hidden">
				<header className="bg-blue-600 text-white text-center py-5 px-4">
					<h1 className="text-2xl md:text-4xl font-bold">{eventoData.evento}</h1>
				</header>

				<main className="p-4 md:p-6 grid grid-cols-2 md:grid-cols-4 gap-4">
					<div className="col-span-2 md:col-span-4">
						<span className="font-bold block mb-2">Hola, {`${obtenerSaludo()} ${data.contacto.nombre}`}</span>
						<span>Aqui tienes los detalles de tu invitación, al llegar al evento muestra este código QR</span>
					</div>
					...
					<div className="col-span-2 h-56">
						<EventMap latitud={latitud} longitud={longitud} eventoData={eventoData} />
					</div>
					...
					<div className="bg-blue-200 rounded-lg p-2 py-3 gap-2 flex items-center justify-center">
						...
						<span className="font-medium text-blue-800">{eventoData.fechaInicio}</span>
					</div>

					<div className="bg-blue-200 rounded-lg p-2 py-3 gap-2 flex items-center justify-center">
						<ClockIcon className="text-blue-600 size-4" />
						<span className="font-medium text-blue-800">{formatTime(eventoData.horaInicio)}</span>
					</div>
					...
					<div className="col-span-2 bg-blue-400 rounded-lg p-4 flex items-center justify-center gap-2">
						...
						<span className="font-medium text-white text-center">{eventoData.municipio.municipio}</span>
					</div>
				</main>
