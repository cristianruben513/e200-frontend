...
export function Sidebar({ className }: React.HTMLAttributes<HTMLDivElement>) {
	...
	return (
		<div className={cn('min-h-[calc(100vh-60px)] p-6', className, collapsed ? 'w-20 px-3' : 'w-64 px-6')}>
			<button
				type="button"
			...
			<div className="space-y-4">
				{Object.entries(groupedLinks).map(([categoriaLabel, { icon: Icon, links }]) => (
					<Accordion key={categoriaLabel} type="single" collapsible className="w-full">
						<AccordionItem
							className={cn(
						...
						>
							<AccordionTrigger className="flex items-center">
								...
								<h3 className={cn('font-semibold', collapsed ? 'text-xs hidden' : 'text-sm')}>
									{categoriaLabel}
								</h3>
							</AccordionTrigger>
							<AccordionContent>
								<div className="grid gap-2">
									{links.map((option, index) => (
										<SideBarItem key={index} currentPathname={pathname} href={option.href}>
											<option.icon className={cn('size-4', !collapsed && 'mr-3')} />
